...
const LOGO_URL = '/maciek_pl/img/logo.svg';
...
// Minimalna ingerencja: do wybranych eventów dodano pole `image`.
const SCHEDULE = {
  Poniedziałek: [
    { start: 16,    end: 18, category: 'Indywidualne',  name: 'Michały',  level: 'Zaawansowany',       spots: 0,     image: '/maciek_pl/img/hero-image.png',    desc: 'Trening indywidualny dla osób wymagających większej uwagi i specjalnego podejścia.' },
    { start: 18,   end: 20, category: 'Juniorzy',     name: 'Zwinne Tygrysy',       level: 'Zaawansowany', spots: 1, image: '/maciek_pl/img/hero-image.png', desc: 'Treningi dla dzieci w wieku 13-16 lat. Zajęcia odbywają się na Koronie, Moodzie i Avatarze (Sikorki). Grupa dla zaawansowanych zawodników, startujących w zawodach pucharowych, lub aspirujących do kadry skałkowej.' },
...
const CSS_VARS = `
  :root {
    --sched-dzieci: #c53030; --sched-dzieci-bg: #fff5f5;
    --sched-juniorzy: #2b6cb0; --sched-juniorzy-bg: #ebf8ff;
    --sched-dorosli: #276749; --sched-dorosli-bg: #f0fff4;
    --sched-indywidualne: #b7791f; --sched-indywidualne-bg: #fffff0;

    --orm-primary: #2e8540; --orm-bw: #8e44ad;
    --sched-accent: var(--orm-primary); --sched-accent-weak: rgba(46,133,64,.35);

    --sched-header-bg: var(--orm-primary);
    --sched-header-border: var(--ifm-color-emphasis-300, #bdbdbd);
    --sched-today-header-bg: #256b38;
    --sched-today-col-bg: rgba(46,133,64,.06);

    --sched-border: var(--ifm-color-emphasis-300, #bdbdbd);
    --sched-line: var(--ifm-color-emphasis-200, #e0e0e0);
    --sched-timecol-bg: var(--ifm-color-emphasis-100, #f5f5f5);
    --sched-time-label: var(--ifm-font-color-secondary, #757575);
    --sched-legend-bg: var(--ifm-color-emphasis-100, #f5f5f5);

    --sched-text: var(--ifm-font-color-base, #3a3a3a);
    --sched-subtext: var(--ifm-font-color-secondary, #757575);
    --sched-btn-border: var(--ifm-color-emphasis-300, #bdbdbd);

    --sched-event-text: var(--ifm-font-color-base, #3a3a3a);
    --sched-popup-bg: var(--ifm-background-surface-color, #fff);
    --sched-popup-border: var(--ifm-color-emphasis-300, #bdbdbd);

    --sched-radius-lg: 12px; --sched-radius-md: 8px; --sched-radius-sm: 6px;
    --sched-shadow: 0 1px 3px rgba(0,0,0,.12);
    --sched-shadow-lg: 0 20px 48px rgba(0,0,0,.28), 0 4px 14px rgba(0,0,0,.08);
  }
  [data-theme='dark'] {
    --sched-dzieci: #fc8181; --sched-dzieci-bg: #2d1515;
    --sched-juniorzy: #63b3ed; --sched-juniorzy-bg: #1a2a3d;
    --sched-dorosli: #68d391; --sched-dorosli-bg: #1a2d1f;
    --sched-indywidualne: #f6e05e; --sched-indywidualne-bg: #2d2710;

    --sched-accent: #2e8540; --sched-accent-weak: rgba(46,133,64,.45);
    --sched-header-bg: #255e37; --sched-header-border: #3a3a3a;
    --sched-today-header-bg: #1f4d2b; --sched-today-col-bg: rgba(46,133,64,.10);

    --sched-border: #3a3a3a; --sched-line: #333; --sched-timecol-bg: #1a1a1a; --sched-time-label: #888; --sched-legend-bg: #1a1a1a;

    --sched-text: #e2e8f0; --sched-subtext: #909090; --sched-btn-border: #3a3a3a;
    --sched-event-text: #e2e8f0; --sched-popup-bg: #242424; --sched-popup-border: #3a3a3a;
    --sched-shadow: 0 1px 3px rgba(0,0,0,.4);
    --sched-shadow-lg: 0 24px 56px rgba(0,0,0,.5), 0 6px 18px rgba(0,0,0,.35);
  }

  .sched-fullwidth { width: 100vw; position: relative; left: 50%; right: 50%; margin-left: -50vw; margin-right: -50vw; padding-left: max(24px, calc((100vw - 1400px) / 2)); padding-right: max(24px, calc((100vw - 1400px) / 2)); box-sizing: border-box; }

  @keyframes sched-event-pop { from { opacity: 0; transform: scaleY(.85);} to { opacity: 1; transform: scaleY(1);} }
  .sched-event-pop { animation: sched-event-pop .18s ease both; transform-origin: top center; }

  @keyframes sched-popup-in { from { opacity: 0; transform: translate(-50%,-48%) scale(.93);} to { opacity: 1; transform: translate(-50%,-50%) scale(1);} }
  .sched-popup { animation: sched-popup-in .22s cubic-bezier(.34,1.3,.64,1) both; }
`;
...
function useSlide() {
  ...
  const slide = (direction) => {
    ...
    setOffset(startX);
    requestAnimationFrame(() => requestAnimationFrame(() => { setTransitioning(true); setOffset(0); }));
  };
...
function EventPopup({ event, onClose, isMobile }) {
  ...
  const popup = (
      <>
        {/* Backdrop */}
        <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 9998, backgroundColor: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(3px)', WebkitBackdropFilter: 'blur(3px)' }} />

        {/* Dialog */}
        <div className="sched-popup" role="dialog" aria-modal="true" style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 9999, width: popupWidth, backgroundColor: 'var(--sched-popup-bg)', borderRadius: 'var(--sched-radius-lg)', boxShadow: 'var(--sched-shadow-lg)', overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            {hasImage && (<div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(0,0,0,0.18), rgba(0,0,0,0.28))' }} />)}

            {/* Fallback: logo placówki, tylko jeśli nie ma zdjęcia */}
            {!hasImage && (
            ...
            <button onClick={onClose} aria-label="Zamknij" style={{ position: 'absolute', top: 10, right: 10, background: 'rgba(255,255,255,0.82)', border: 'none', borderRadius: 20, color: '#333', fontSize: '0.85rem', cursor: 'pointer', padding: '4px 10px', lineHeight: 1.4, fontWeight: 700, backdropFilter: 'blur(4px)', WebkitBackdropFilter: 'blur(4px)' }}>✕</button>
          </div>
          ...
          <div style={{ padding: bodyPad, backgroundColor: 'var(--sched-popup-bg)' }}>
            <div style={{ display: 'flex', gap: isMobile ? 14 : 24, marginBottom: 10, flexWrap: 'wrap' }}>
              <div style={{ fontSize: metaSize, color: 'var(--sched-subtext)' }}>🕐 <strong style={{ color: 'var(--sched-text)' }}>{fmt(event.start)} – {fmt(event.end)}</strong></div>
              <div style={{ fontSize: metaSize, color: 'var(--sched-subtext)' }}>🏔️ <strong style={{ color: 'var(--sched-text)' }}>{event.level}</strong></div>
            </div>
            ...
            {event.spots > 0 && (
                <a href="/kontakt" style={{ display: 'block', width: '100%', boxSizing: 'border-box', padding: isMobile ? '12px 16px' : '13px 18px', borderRadius: 12, textAlign: 'center', backgroundColor: lightCat.color, color: '#fff', fontWeight: 700, fontSize: isMobile ? '0.95rem' : '1rem', textDecoration: 'none', transition: 'filter 0.15s, transform 0.15s' }}
      ...
      <>
        ...
        <div style={{ position: 'relative', height: HOURS.length * DESKTOP_SLOT_HEIGHT, borderLeft: '1px solid var(--sched-border)', backgroundColor: isToday ? 'var(--sched-today-col-bg)' : 'transparent' }}>
          ...
          {events.map((ev, idx) => {
            ...
            return (
                <div key={idx} onClick={() => setPopup(ev)} style={{ position: 'absolute', top, height, left: 3, right: 3, backgroundColor: `var(${cat.bgVar})`, borderLeft: `3px solid var(${cat.colorVar})`, borderRadius: 'var(--sched-radius-md)', padding: '3px 6px', overflow: 'hidden', boxShadow: 'var(--sched-shadow)', cursor: 'pointer', zIndex: 1, transition: 'box-shadow 0.15s, transform 0.15s' }}
      ...
      <>
        ...
        <div style={{ position: 'relative', height: HOURS.length * slotH, borderLeft: '1px solid var(--sched-border)' }}>
          ...
          {events.map((ev, idx) => {
            ...
            return (
                <div key={idx} className="sched-event-pop" onClick={() => setPopup(ev)} style={{ position: 'absolute', top, height, left: 3, right: 3, backgroundColor: `var(${cat.bgVar})`, borderLeft: `3px solid var(${cat.colorVar})`, borderRadius: 'var(--sched-radius-sm)', padding: '2px 5px', overflow: 'hidden', boxShadow: 'var(--sched-shadow)', cursor: 'pointer', zIndex: 1, animationDelay: `${idx * 25}ms` }}>
                  <div style={{ fontSize: '0.67rem', fontWeight: 700, color: `var(${cat.colorVar})`, lineHeight: 1.2 }}>
                    {fmt(ev.start)}–{fmt(ev.end)}
                  </div>
                  {height >= 28 && (<div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--sched-event-text)', lineHeight: 1.2, overflow: 'hidden' }}>{ev.name}</div>)}
...
function MobileView({ filter }) {
  ...
  useEffect(() => {
    const calc = () => {
      ...
      const reserved = 52 + 42 + 42 + navH + dotsH + 32 + 32;
  ...
  const slideStyle = { transform: `translateX(${offset}%)`, transition: transitioning ? 'transform 0.35s cubic-bezier(0.4,0,0.2,1), opacity 0.35s ease' : 'none', opacity: transitioning ? 1 : (offset === 0 ? 1 : 0) };

  return (
      <div>
        <div ref={navRef} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
          <button onClick={() => dayIndex > 0 && goTo(dayIndex - 1, 'right')} disabled={dayIndex === 0} style={{ padding: '5px 13px', borderRadius: 8, border: '1px solid var(--sched-btn-border)', backgroundColor: 'transparent', color: dayIndex === 0 ? 'var(--sched-btn-border)' : 'var(--sched-text)', cursor: dayIndex === 0 ? 'default' : 'pointer', fontSize: '1.1rem' }}>‹</button>
          <div style={{ textAlign: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'center' }}>
              <span style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--sched-text)' }}>{day}</span>
              {isToday && (<span style={{ fontSize: '0.62rem', fontWeight: 700, color: '#fff', backgroundColor: 'var(--sched-accent)', borderRadius: 10, padding: '1px 7px' }}>DZIŚ</span>)}
            </div>
            <div style={{ fontSize: '0.65rem', color: 'var(--sched-subtext)' }}>{dayIndex + 1} / {DAYS.length} · przesuń aby zmienić</div>
          </div>
          <button onClick={() => dayIndex < DAYS.length - 1 && goTo(dayIndex + 1, 'left')} disabled={dayIndex === DAYS.length - 1} style={{ padding: '5px 13px', borderRadius: 8, border: '1px solid var(--sched-btn-border)', backgroundColor: 'transparent', color: dayIndex === DAYS.length - 1 ? 'var(--sched-btn-border)' : 'var(--sched-text)', cursor: dayIndex === DAYS.length - 1 ? 'default' : 'pointer', fontSize: '1.1rem' }}>›</button>
        </div>

        <div ref={dotsRef} style={{ display: 'flex', justifyContent: 'center', gap: 5, marginBottom: 6 }}>
          {DAYS.map((_, i) => (
              <div key={i} onClick={() => i !== dayIndex && goTo(i, i > dayIndex ? 'left' : 'right')} style={{ width: i === dayIndex ? 18 : 7, height: 7, borderRadius: 4, backgroundColor: i === dayIndex ? (i === todayIdx ? 'var(--sched-accent)' : 'var(--sched-header-bg)') : (i === todayIdx ? 'var(--sched-accent-weak)' : 'var(--sched-btn-border)'), cursor: 'pointer', transition: 'all 0.3s cubic-bezier(0.4,0,0.2,1)' }} />
          ))}
        </div>

        <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} style={{ overflow: 'hidden', touchAction: 'pan-y', borderRadius: 10, border: '1px solid var(--sched-border)' }}>
          <div style={slideStyle}>
            <div style={{ display: 'grid', gridTemplateColumns: '44px 1fr' }}>
              ...
              <div style={{ backgroundColor: isToday ? 'var(--sched-today-header-bg)' : 'var(--sched-header-bg)', color: '#fff', padding: '6px 4px', textAlign: 'center', fontSize: '0.78rem', fontWeight: 700, borderLeft: '1px solid var(--sched-header-border)' }}>{day}{isToday && ' ✦'}</div>
  ...
  return (
      <div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 12 }}>
          <button onClick={() => setSelectedDay(null)} style={{ padding: '5px 12px', borderRadius: 8, border: '1px solid var(--sched-btn-border)', backgroundColor: selectedDay === null ? 'var(--sched-header-bg)' : 'transparent', color: selectedDay === null ? '#fff' : 'var(--sched-subtext)', fontSize: '0.78rem', fontWeight: selectedDay === null ? 700 : 400, cursor: 'pointer', transition: 'all 0.2s' }}>Cały tydzień</button>
          {DAYS.map((d, i) => (
              <button key={d} onClick={() => setSelectedDay(selectedDay === i ? null : i)} style={{ padding: '5px 12px', borderRadius: 8, border: `1px solid ${i === todayIdx && selectedDay !== i ? 'var(--sched-accent-weak)' : 'var(--sched-btn-border)'}`, backgroundColor: selectedDay === i ? 'var(--sched-header-bg)' : 'transparent', color: selectedDay === i ? '#fff' : 'var(--sched-subtext)', fontSize: '0.78rem', fontWeight: selectedDay === i ? 700 : 400, cursor: 'pointer', transition: 'all 0.2s', position: 'relative' }}>
                {DAYS_SHORT[i]}
                {i === todayIdx && (<span style={{ position: 'absolute', top: -5, right: -4, width: 7, height: 7, borderRadius: '50%', backgroundColor: 'var(--sched-accent)', border: '1.5px solid var(--sched-popup-bg)' }} />)}
              </button>
          ))}
        </div>
  ...
  return (
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: isMobile ? 6 : 8, marginBottom: 10, padding: isMobile ? '8px 12px' : '10px 14px', backgroundColor: 'var(--sched-legend-bg)', borderRadius: 8, border: '1px solid var(--sched-border)' }}>
        ...
        {['Wszystkie', ...Object.keys(CATEGORIES)].map((key) => {
          ...
          return (
              <button key={key} onClick={() => setFilter(key)} style={{ padding: isMobile ? '4px 11px' : '5px 14px', borderRadius: 20, border: 'none', outline: 'none', cursor: 'pointer', transition: 'all 0.18s', whiteSpace: 'nowrap', fontSize: isMobile ? '0.75rem' : '0.82rem', fontWeight: active ? 700 : 400, backgroundColor: active ? (cat ? `var(${cat.colorVar})` : 'var(--sched-header-bg)') : 'var(--sched-btn-border)', color: active ? '#ffffff' : 'var(--sched-subtext)', boxShadow: active ? '0 2px 8px rgba(0,0,0,0.15)' : 'none' }}>
                {cat ? `${cat.emoji} ` : ''}{key}
              </button>
          );
        })}
